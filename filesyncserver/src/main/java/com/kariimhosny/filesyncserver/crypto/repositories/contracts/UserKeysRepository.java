...
/**
 * Repository interface for UserKey entity operations. Handles public key
 * management for users including key rotation scenarios.
 */
@Repository
public interface UserKeysRepository extends JpaRepository<UserKey, Long> {
    ...
    /**
     * get all space users' active public keys
     * @param spaceId 
     * @return 
     */
    ...
    // ===== Bulk Operations =====
    /**
     * Deactivate all keys for a specific user Useful before adding a new active
    ...
    /**
     * Deactivate all keys except the specified one Useful when promoting a key
     * to active status
     */
    @Modifying
    @Transactional
